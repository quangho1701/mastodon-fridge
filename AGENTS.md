...
**Tech stack:** React Native (Expo ~54), TypeScript, Firebase Auth (planned), Codex Vision API (Anthropic, planned for backend).
...
- **AI Scanner (ScanScreen → ConfirmScreen):** Camera capture → compress → POST to backend `/api/extract` (Codex Vision) → editable confirmation form. Backend not yet built.
